						...
						<div class="block">
							<h3>
								...
								if (isset($_GET['categorie'])) {
									$title_categori_q = mysqli_query($connection, "SELECT (`title`) AS `title_categori` FROM `articals categori` WHERE `id` = " . $_GET['categorie']);
									$title_categori = mysqli_fetch_assoc($title_categori_q);
									$title_categori = $title_categori['title_categori'];
									echo $title_categori;
							...
							<div class="block_content">
								<div class="articles articles_horizontal">
									...
									$filters = "";
									if (isset($_GET['categorie'])) {
										$filters .= " WHERE `categorie_id`  =  " . $_GET['categorie'];
									}



									$total_count_q = mysqli_query($connection, "SELECT COUNT(`id`) AS `total_count` FROM `articles` " . $filters);
									...
									$articles = mysqli_query($connection, "SELECT * FROM `articles` $filters ORDER BY `id` DESC LIMIT $offset,$per_page");
